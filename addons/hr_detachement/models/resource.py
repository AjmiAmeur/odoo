...
class ResourceCalendar(models.Model):
    ...
    detachements_count = fields.Integer("# detachements using it", compute='_compute_detachements_count', groups="hr_detachement.group_hr_detachement_manager")
    ...
    def _compute_detachements_count(self):
        count_data = self.env['hr.detachement']._read_group(
        ...
        for calendar in self:
            calendar.detachements_count = mapped_counts.get(calendar.id, 0)

    def action_open_detachements(self):
        self.ensure_one()
